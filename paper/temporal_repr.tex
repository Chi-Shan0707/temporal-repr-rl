...
Using Centered Kernel Alignment (CKA) against engineered temporal encodings, linear probing, and PCA on a periodic gridworld, we ask three questions: (Q1) Is temporal structure learned or inherent to recurrence?
(Q2) Is the emergent encoding unique across training runs?
(Q3) How does the encoding depend on the temporal horizon?
Untrained LSTM controls show weak temporal structure, which emerges reliably after RL training.
Multi-seed experiments reveal that the learned encoding is \emph{non-unique}: the highest-CKA engineered encoding varies across seeds, from sinusoidal positional encoding to phase-based encoding.
...
\textbf{No canonical internal clock.}
...
The representational plurality we observe raises a question for RL interpretability: if the temporal code is not unique, what does it mean to ``interpret'' a neuron as a time cell?
...
\textbf{What does ``representation'' mean in RL?}
...
If the code is path-dependent, then any single-seed analysis characterizes one solution among many, not a canonical internal structure.
Whether this plurality is a nuisance for interpretability or a genuine feature of learning systems is open.

\textbf{Scope and limitations.}
We do not claim a universal taxonomy of temporal coding in RL.
Instead, we isolate one setting where temporal structure can be controlled and representation geometry inspected directly.
Three caveats:
...
Whether representational plurality persists across architectures (GRUs, attention-based models) and in more complex domains remains to be tested.
