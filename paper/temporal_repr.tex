...
\contribution{We show that temporal encoding in recurrent RL agents is not unique on complex environments---different training runs produce different internal clocks---but converges to a single code on simpler ones.}{Challenges the ``canonical temporal code'' assumption and links plurality to environment complexity.}
...
\begin{abstract}
Recurrent RL agents without an explicit clock must invent their own internal representation of time.
A natural assumption is that this representation is unique: any sufficiently trained agent converges to the same canonical temporal code.
We show this assumption does not hold.
LSTM agents trained on the same periodic task construct different temporal codes depending on their random initialization, despite identical behavioral performance.
Using CKA, linear probing, and PCA on two periodic environments---a 2D gridworld (DoorGrid) and a 1D ring (Cyclic Corridor)---we ask three questions: (Q1) Is temporal structure learned or inherent to recurrence?
(Q2) Is the emergent encoding unique across training runs?
(Q3) How does the encoding depend on the temporal horizon?
On DoorGrid, the learned encoding is \emph{non-unique}: the highest-CKA engineered encoding varies across seeds, from sinusoidal positional encoding to phase-based encoding.
On Cyclic Corridor, all seeds converge to the same encoding, suggesting that plurality depends on environment complexity.
A controlled horizon manipulation shows that the encoding is also \emph{horizon-sensitive}: at short horizons, agents align with sinusoidal codes; at longer horizons, alignment shifts toward scalar encoding.
These findings hold implications for how we interpret recurrent representations: if there is no single canonical temporal code, single-seed representational analyses may not generalize.
\end{abstract}
...
The answer is no: different training runs produce different temporal codes despite identical behavioral performance.
...
The \emph{variability and contingency} of such representations in recurrent RL agents has received less attention.
...
The temporal framing learned by LSTM agents is \emph{non-unique}: agents with identical task performance align differently with simple engineered temporal codes, and the preferred alignment shifts with temporal horizon.
This challenges how we interpret and compare recurrent representations across training runs---if there is no single canonical code, what exactly are we comparing?
...
This minimal environment isolates temporal encoding from complex spatial navigation: the periodic structure is the agent's primary challenge.

\textbf{Cyclic Corridor} is a 1D ring of 32 cells with a periodic door at cell 15 (open during the first half of each period, closed during the second half).
The goal is at cell 26; the agent starts at cell 0 and can move left or right (wrapping around).
With $T = 16$, the state space is $32 \times 16 = 512$.
This environment strips away all spatial complexity: there is no 2D navigation, no branching paths---only a periodic obstacle on a ring.
If representational plurality depends on the complexity of the spatial task, Cyclic Corridor should produce more uniform encoding across seeds than DoorGrid.
...
Temporal structure is training-dependent, not an artifact of random recurrent dynamics.
The MLP baseline (which lacks recurrence) fails to learn the task (Return $= -2.0$ across all 5 seeds, R$^2(t) \approx 0$), confirming that temporal tracking requires recurrent state.
...
The top-8 phase-selective dimensions show low overlap across seeds (1.8/8, Jaccard $= 0.14$), but this is indistinguishable from chance given that 34 of 64 dimensions have near-maximal selectivity (hypergeometric expectation $\approx 1.88$).
We do not treat single-neuron overlap as evidence for or against plurality; the population-level CKA differences provide the stronger signal.

\subsection{Plurality Depends on Environment Complexity}
\label{sec:cyclic}

\begin{table}[t]
\centering
\caption{CKA (mean $\pm$ std across 5 seeds) on Cyclic Corridor ($T = 16$).
All seeds converge to phase sin/cos as the best match---unlike DoorGrid, where the preferred encoding varies across seeds.}
\label{tab:cyclic}
\begin{tabular}{lc}
\toprule
\textbf{Encoding} & \textbf{CKA (mean $\pm$ std)} \\
\midrule
Raw scalar ($t/T$)            & $0.885 \pm 0.010$ \\
Sinusoidal PE                 & $0.760 \pm 0.007$ \\
Phase sin/cos                 & $\mathbf{0.896 \pm 0.010}$ \\
One-hot phase                 & $0.471 \pm 0.005$ \\
\bottomrule
\end{tabular}
\end{table}

To test whether representational plurality is specific to environments with spatial complexity, we repeat the multi-seed experiment on Cyclic Corridor.
Table~\ref{tab:cyclic} shows a striking contrast with DoorGrid: all 5 seeds agree on phase sin/cos as the best-matching encoding, with low variance (CKA std $= 0.010$ vs.\ $0.21$ for phase sin/cos on DoorGrid).
The 1D ring eliminates spatial ambiguity, and with it, the seed-dependent encoding variability observed on DoorGrid.
This suggests that representational plurality emerges when the environment offers multiple viable encoding strategies---not as a universal property of recurrent networks.
...
At $T = 32$, training becomes less stable: 2 of 5 seeds fail to converge, and task return drops to $0.640$ (see Appendix~\ref{app:failed} for failed-seed training curves).
...
LSTM-based RL agents do not converge to a single canonical temporal code---at least not in environments with spatial complexity.
On DoorGrid, the code depends on initialization, and which engineered encoding it resembles shifts with the task's temporal horizon.
On the simpler Cyclic Corridor, all seeds converge to the same encoding (phase sin/cos), suggesting that plurality is a property of the environment, not of the learning algorithm.
These findings hold implications for how we interpret recurrent representations.

\textbf{Plurality is environment-dependent.}
The assumption that recurrent networks provide a unified, well-defined temporal code is embedded in much of RL practice.
Our results show this assumption is environment-dependent: on DoorGrid, the framing of time is contingent on optimization path, but on Cyclic Corridor, all seeds converge to the same code.
Plurality arises when the environment offers multiple viable encoding strategies, not as a universal property of recurrent learning.

\textbf{Interpretability and representation.}
...
This matches \citet{edelman2001degeneracy}'s concept of degeneracy---structurally different elements performing the same function---and suggests that single-seed interpretability analyses may be misleading.
A study using only seed 42 would conclude ``LSTMs encode time via sinusoidal-like codes,'' while seed 456 would suggest phase-based codes.
Neither generalizes.

\textbf{Limitations.}
Two caveats: first, our experiments use two environments, one algorithm (REINFORCE), and one architecture (single-layer LSTM).
Second, CKA measures linear similarity and may miss nonlinear encoding strategies.
Whether representational plurality persists across architectures and in more complex domains remains to be tested.
