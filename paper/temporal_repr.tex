...
A natural assumption is that this framing is unique: any sufficiently trained agent converges to the same canonical temporal code.
We show this assumption does not hold: LSTM agents trained on the same periodic task construct different internal representations of time depending on their random initialization.
...
Multi-seed experiments ($T = 4$, 5 seeds) reveal that the learned encoding is \emph{non-unique}: all agents achieve perfect timestep prediction (R$^2 = 1.0$) yet exhibit different CKA profiles. The highest-CKA engineered encoding varies across seeds: sinusoidal positional encoding dominates in 4 of 5 seeds (CKA $0.64$--$0.74$), while seed 456 aligns more strongly with phase-based encoding (CKA $= 0.62$).
...
These findings have implications for how we interpret recurrent representations.
